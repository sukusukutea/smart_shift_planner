class FixSlotOnShiftDayAssignments < ActiveRecord::Migration[8.1]
  def change
    unless column_exists?(:shift_day_assignments, :slot)
      add_column :shift_day_assignments, :slot, :integer, null: false, default: 0
    end

    if index_exists?(:shift_day_assignments, name: "idx_sda_draft_unique")
      remove_index :shift_day_assignments, name: "idx_sda_draft_unique"
    end

    if index_exists?(:shift_day_assignments, name: "idx_sda_confirmed_unique")
